...
class Forecasting_algorithm {
 ...
 public:
  std::map<std::string, std::vector<OutType>>
  operator () (const std::vector<InType> &time_series,
 ...
 protected:
   ...
   *
   */
  virtual itp::Pointwise_predictor_ptr<OutType, InType>
  make_predictor(itp::CodeLengthsComputerPtr<OutType> computer, itp::SamplerPtr<InType> sampler,
                 size_t difference) const = 0;
};

template <typename OutType, typename InType>
std::map<std::string, std::vector<OutType>>
Forecasting_algorithm<OutType, InType>::operator () (const std::vector<InType> &time_series,
                                                     const itp::Names &compressors_groups, size_t horizont,
                                                     size_t difference, int sparse) {
  ...
  itp::Pointwise_predictor_ptr<OutType, InType> pointwise_predictor = make_predictor(computer,
                                                                                     sampler, difference);
  if (sparse > 0) {
    pointwise_predictor = std::make_shared<itp::Sparse_predictor<OutType, InType>>(pointwise_predictor, sparse);
  }

  itp::Forecast<OutType> res = pointwise_predictor->Predict(time_series, horizont, compressors);
  std::map<std::string, std::vector<OutType>> ret;
  for (const auto &compressor : res.get_index()) {
    ret[compressor] = std::vector<OutType>(horizont);
...
class Forecasting_algorithm_discrete : public Forecasting_algorithm<DoubleT, SymbolT> {
 protected:
  itp::Pointwise_predictor_ptr<DoubleT, SymbolT> make_predictor(itp::CodeLengthsComputerPtr<DoubleT> computer,
...
class Forecasting_algorithm_real : public Forecasting_algorithm<DoubleT, DoubleT> {
 ...
 protected:
  itp::Pointwise_predictor_ptr<DoubleT, DoubleT> make_predictor(itp::CodeLengthsComputerPtr<DoubleT> computer,
...
Forecasting_algorithm_discrete<DoubleT, SymbolT>::make_predictor(itp::CodeLengthsComputerPtr<DoubleT> computer,
                                                                 itp::SamplerPtr<SymbolT> sampler,
...
Forecasting_algorithm_real<DoubleT>::make_predictor(itp::CodeLengthsComputerPtr<DoubleT> computer,
                                                    itp::SamplerPtr<DoubleT> sampler, size_t difference) const {
...
template <typename DoubleT>
itp::Pointwise_predictor_ptr<DoubleT, DoubleT>
Forecasting_algorithm_multialphabet<DoubleT>::make_predictor(itp::CodeLengthsComputerPtr<DoubleT> computer,
                                                             itp::SamplerPtr<DoubleT> sampler,
                                                             size_t difference) const {
