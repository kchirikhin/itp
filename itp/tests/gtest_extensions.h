...
void ExpectTransformedContainerEq(
        const Container1& exact_container,
        const Container2& container_to_transform,
        const Op& op)
{
    ASSERT_EQ(exact_container.size(), container_to_transform.size());

    auto p = std::cbegin(exact_container);
    auto q = std::cbegin(container_to_transform);
    while (p != std::cend(exact_container))
    {
        EXPECT_EQ(*p++, op(*q++));
    }
}
