 ...
 *
 * \param[in] sym the symbol which probability is needed to evaluate;
 * \param[in] sym_freq how many times sym was encountered in the word;
 * \param[in] total_freq is the position of sym in the word;
 * \param[in] alphabet_size is the number of all possible symbols which may be found in the word.
 */
...
public:
	 ...
	 *
	 * \param[in] w PlainTimeSeries<Symbol> to evaluate probability.
	 *
...
public:
	...
	enum class IsPredictionConfident
	{
		Yes,
		No
	};

	MultiheadAutomaton(Symbol alphabet_min_symbol, Symbol alphabet_max_symbol);
...
protected:
	/**
	 * Sets name to the head with specified number.
	 *
	 * \param[in] head_kNo Number of head.
	 * \param[in] name Name of the head.
	 */
	void SetHeadName(size_t head_index, const std::string& name);
	 ...
	 *
	 * \param[in] head_to_move Head which should be moved.
	 *
	...
	AutomatonWord a; /**< Time series to forecast (name chose to be close
						  to the paper. */
...
template<size_t N>
itp::MultiheadAutomaton<N>::MultiheadAutomaton(Symbol alphabet_min_symbol, Symbol alphabet_max_symbol)
	: alphabet_min_symbol_{alphabet_min_symbol}
	, alphabet_max_symbol_{alphabet_max_symbol}
	, letters_freq_(alphabet_max_symbol_ + 1)
	, confident_guess_freq_(alphabet_max_symbol_ + 1)
{
...
template<size_t N>
void itp::MultiheadAutomaton<N>::SetHeadName(size_t head_index, const std::string& name)
{
	assert(head_index < N);
	heads_[head_index].Name(name);
}

template<size_t N>
void itp::MultiheadAutomaton<N>::Guess(Symbol guessed_symbol, IsPredictionConfident confidence)
{
	size_t total_freq;
	...
	if (h(num_of_rightmost_head_) < a.size() - 1)
	{
		...
		switch (confidence)
		{
		case IsPredictionConfident::Yes:
...
{
	std::for_each(std::begin(heads_), std::end(heads_), [](Head& head) { head.Move(-1); });
