...
{
protected:
	static constexpr size_t count_of_intervals_to_split = 2;
...
{
	EXPECT_THAT(sampler_.Transform({}, count_of_intervals_to_split), IsEmpty());
}

TEST_F(SamplerForDoublesTest, ThrowsOnASeriesWithSingleValue)
{
	EXPECT_THROW(sampler_.Transform({1.}, count_of_intervals_to_split), SeriesTooShortError);
}

TEST_F(SamplerForDoublesTest, QuantizesASeriesWithNegativeValues)
{
	EXPECT_THAT(sampler_.Transform({0.1, -0.5, 5, -1, 7.5}, count_of_intervals_to_split), ElementsAre(0, 0, 1, 0, 1));
}
...
TEST_F(SamplerForDoublesTest, CorrectlyDequantizesASeries)
{
	...
	auto transformed_series = sampler_.Transform({0.1, -0.5, 5, -1, 7.5}, count_of_intervals_to_split);
...
{
	EXPECT_THAT(
		sampler_.Transform({{0.4, 1.4}, {1.2, 1.6}, {0.6, 1.4}, {1.8, 1.8}}, kCountOfIntervalsToSplit),
		ElementsAre(0, 3, 0, 3));
}
...
TEST_F(SamplerForVectorDoublesTest, ThrowsIfSizeOfAlphabetExceeds256)
{
	...
	EXPECT_THROW(
		sampler_.Transform({{0.4, 1.4}, {1.2, 1.6}, {0.6, 1.4}, {1.8, 1.8}}, kLargeIntervalsCount),
		IntervalsCountError);
}
...
TEST(PointwiseMinElementsTest, ReturnsEmptyVectorOnEmptyInput)
{
	...
	ExpectContainerEmpty(PointwiseMinElements(std::begin(empty_vec), std::end(empty_vec)));
}

TEST(PointwiseMinElementsTest, CorrectlyWorksOnValidData)
{
	std::vector<VectorSymbol> vec{{4, 2}, {1, 3}};
	ExpectContainersEq(PointwiseMinElements(std::begin(vec), std::end(vec)), std::vector<Symbol>{1, 2});
}

TEST(PointwiseMaxElementsTest, CorrectlyWorksOnValidData)
{
	std::vector<VectorSymbol> vec{{4, 2}, {1, 3}};
	ExpectContainersEq(PointwiseMaxElements(std::begin(vec), std::end(vec)), std::vector<Symbol>{4, 3});
}
