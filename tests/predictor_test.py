...
class TestPredictor(unittest.TestCase):
    ...
    def test_basic_forecast(self):
        ...
        forecast = self._itp.forecast_multialphabet(ts, groups=["zlib_rp"], h=2, max_quants_count=4, difference=0)
    ...
    def test_discrete_forecast(self):
        ...
        forecast = self._itp.forecast_discrete(ts.tolist(), groups=["zlib_rp"], h=2, difference=0)
        np.testing.assert_array_almost_equal(np.array(forecast["zlib_rp"]),
    ...
    def test_kindex_with_automaton(self):
        ts = np.array(
        ...
        forecast = self._itp.forecast_discrete(ts.tolist(), groups=["zlib"], h=24, difference=0, sparse=8)
