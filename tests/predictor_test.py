...
class TestPredictor(unittest.TestCase):
    ...
    def test_basic_forecast(self):
        ...
        forecast = self._itp.make_forecast_multialphabet(ts, groups=["zlib_rp"], h=2, max_quants_count=4, difference=0)
    ...
    def test_discrete_forecast(self):
        ...
        forecast = self._itp.make_forecast_discrete(ts.tolist(), groups=["zlib_rp"], h=2, difference=0)
        np.testing.assert_array_almost_equal(np.array(forecast["zlib_rp"]),
    ...
    def test_m3c_year(self):
        ...
        forecast = self._itp.make_forecast_multialphabet(ts, groups=["zlib_rp"], h=6, max_quants_count=4, sparse=2,
                                                         difference=1)
        np.testing.assert_array_almost_equal(np.array(forecast["zlib_rp"]),
        ...
        forecast = self._itp.make_forecast_multialphabet(ts, groups=["zstd_ppmd"], h=6, max_quants_count=4, sparse=2,
                                                         difference=1)
    ...
    def test_kindex_with_automation(self):
        ...
        forecast = self._itp.make_forecast_discrete(ts.tolist(), groups=["zlib"], h=24, difference=0, sparse=8)
